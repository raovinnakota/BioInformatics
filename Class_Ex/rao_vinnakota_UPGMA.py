...
def closest_neighbors(distMatrix):
    min_dist = 100000000000
    for i in distMatrix:
        for j in i:
            if (j < min_dist and j != 0.0):
                min_dist = j
                column = j
                row = i
    return (min_dist, row, column)

print (closest_neighbors(ourdistMatrix))
#Tree construction
def getDistToJunction(distMatrix):
  n = len(distMatrix)
  min_dist, row, column = closest_neighbors(distMatrix)
